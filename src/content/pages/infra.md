...
![IX2215 の ACL 設定出力。blockl2(EtherType)、bdhcp(IPv4)、oldlan(IPv4・IPv6 の両方)の各リストが定義されている](/images/acl-output.png)

フィルタは大きく3系統に分かれています。

- **blockl2** — 特定 EtherType のL2フレームを遮断
- **bdhcp** — 学内LAN側からの DHCP(udp/67-68)を遮断し、[問題2](/problems/) のレース応答を PoC セグメントに持ち込ませない
- **oldlan** — NetBIOS、SNMP、SSDP、WS-Discovery、mDNS、LLMNR、プリンタ探索(BJNP 等)、Dropbox LAN Sync といった探索系トラフィックを IPv4/IPv6 の両方で遮断

この抑制の効果は [現状の課題](/problems/) に載せた実測動画のとおり、無関係パケット 75% → 8% 前後です。
...
![Grafana ダッシュボード。IX2215 のインターフェース状態とトラフィック、C9800 と AP の状態、Proxmox のリソース、Unbound の QPS とキャッシュヒット率が1画面に並ぶ](/images/grafana.png)
...
![Proxmox VE の管理画面。VM 100 (c9800) と LXC 101 (unbound)、102 (local-server) が1ノードに同居](/images/proxmox.png)
